  ...
  async verificarB(form) {
    this.authService.VerificarUser(form).subscribe((data) => {
      ...
      if (data.valid == "OK") {
        ...
        this.shoppingService.showCart(info).subscribe(
          (data) => {
            console.log(data);
        ...
        this.mensajeIncorrecto(
          "Acceso Incorrecto",
          "Algo salió mal su correo o contraseña están incorrectos"
        );
  ...
  facebook() {
    this.authService
      ...
      .then((res) => {
        ...
        const log = { correo: mail, contrasena: contra };
      ...
      .catch((err) => {
        this.mensajeIncorrecto(
          "Fallo de conexión",
          "Algo salio mal no se pudo iniciar sesión"
        );
      });
  }
